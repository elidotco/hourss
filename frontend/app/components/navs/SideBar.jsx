...
const SideBar = ({ sidebarOpen }) => {
  ...
  const toggleDropdown = (dropdownId) => {
    // Toggle between opening and closing the current dropdown
    if (activeDropdown === dropdownId) {
      setActiveDropdown(null); // Close it if it's already open
    } else {
  ...
  return (
    <aside
      className={` bg-sidebar_color z-99 transition-all duration-300 ease-in-out text-white h-screen fixed  overflow-scrolltop-0 left-0 ${
                  ...
                  >
                    {nav.subs.map((items) => (
                      <Link key={items.name} href={items.path}>
                        <li className=" p-1 sub text-gray-300 hover:text-blue-300 cursor-pointer rounded">
                          {items.name}
                        </li>
                      </Link>
                    ))}
                ...
                <Link href={nav.path}>
                  <div className="flex items-center ml-3  gap-3">
                    <div
                      ...
                      } rounded-[3px] ${sidebarOpen ? "mx-atuo" : ""}  `}
                    >
                      <nav.icon className="w-5 h-5" size={18} />
                    </div>
                    <p
                      className={`${
                        sidebarOpen
                          ? "hidden group-hover:flex g9 group-hover:text-[#ff5714]   hover:flex  absolute left-[100%] rounded-r-[3px] bg-sidebar_color p-4 w-[12rem] transition duration-200 ease-in-out pointer-events-none group-hover:pointer-events-auto"
                          : ""
                      } ${
                        pathname.includes(nav.path)
                          ? "  text-white"
